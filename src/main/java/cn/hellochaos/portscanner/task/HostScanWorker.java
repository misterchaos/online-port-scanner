...
public class HostScanWorker implements Runnable {
  ...
  public HostScanWorker(
      HostInfo hostInfo, CountDownLatch hostCountDownLatch, int minPort, int maxPort) {
    this.hostInfo = hostInfo;
    this.hostCountDownLatch = hostCountDownLatch;
  ...
  public void run() {
    ...
    try {
      InetAddress inetAddress = InetAddress.getByName(hostInfo.getIp());
    ...
    for (int i = minPort; i <= maxPort; i++) {
      ...
      ScanExecutor.submit(new PortScanWorker(portInfo, hostCountDownLatch));
    }
  }
}
